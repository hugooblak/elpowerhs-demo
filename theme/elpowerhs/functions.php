<?php
/**
 * El Power HS AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "El Power HS AB Leads" plugin, so leads survive a theme change.
 *
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'elpowerhs', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'elpowerhs' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'elpowerhs' ) ) );

		register_block_pattern_category( 'elpowerhs-sections', array( 'label' => __( 'El Power HS AB sections', 'elpowerhs' ) ) );
		register_block_pattern_category( 'elpowerhs-pages', array( 'label' => __( 'El Power HS AB page layouts', 'elpowerhs' ) ) );
	}
);
