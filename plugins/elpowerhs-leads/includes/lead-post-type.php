...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'elpowerhs-leads' ),
					'singular_name' => __( 'Lead', 'elpowerhs-leads' ),
					'edit_item'     => __( 'Förfrågan', 'elpowerhs-leads' ),
					'search_items'  => __( 'Search leads', 'elpowerhs-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'elpowerhs-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'elpowerhs-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'elpowerhs-leads' ),
			'npl_phone'    => __( 'Phone', 'elpowerhs-leads' ),
			'npl_service'  => __( 'Needs', 'elpowerhs-leads' ),
			'npl_timeline' => __( 'When', 'elpowerhs-leads' ),
			'npl_zip'      => __( 'ZIP', 'elpowerhs-leads' ),
			'npl_source'   => __( 'Source', 'elpowerhs-leads' ),
			'npl_status'   => __( 'Status', 'elpowerhs-leads' ),
			'date'         => __( 'Received', 'elpowerhs-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'elpowerhs-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'elpowerhs-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'elpowerhs-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'elpowerhs-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'elpowerhs-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'elpowerhs-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'elpowerhs-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'elpowerhs-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'elpowerhs-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'elpowerhs-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'elpowerhs-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'elpowerhs-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
