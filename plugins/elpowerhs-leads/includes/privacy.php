...
add_filter(
	...
	function ( $exporters ) {
		$exporters['elpowerhs-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'elpowerhs-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'elpowerhs-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'elpowerhs-leads',
						'group_label' => __( 'Quote requests', 'elpowerhs-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['elpowerhs-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'elpowerhs-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
